...
if annotator == "admin":
    ...
    view_file = st.selectbox(
        ...
        key="admin_file_selector"
    )

    # avoid stale switching issues
    if (
        "admin_file" not in st.session_state
        or st.session_state.admin_file != view_file
    ):
        ...
        if os.path.exists(view_file):
            ...
            st.warning(f"{view_file} not found")
            loaded_data = []

        # IMPORTANT: separate admin dataset from annotator dataset
        st.session_state.data = copy.deepcopy(loaded_data)
        st.session_state.admin_file = view_file
        st.session_state.current_idx = None

    # LOCK MODE: prevent accidental annotation edits
    st.session_state.annotator = "admin"
    ...
    if not os.path.exists(base_file):
        st.error(f"Missing dataset file: {base_file}")
        st.stop()

    # load progress or base dataset
...
# =========================
# SESSION STATE FIX (NO DATA LEAK)
# =========================
if (
    "data" not in st.session_state
    or st.session_state.get("annotator") != annotator
):
